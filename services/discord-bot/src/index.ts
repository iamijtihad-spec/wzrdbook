...
// Load Envs
// 1. Service-level .env (Bot Token, Guild ID)
const localEnvPath = path.resolve(__dirname, '../.env');
dotenv.config({ path: localEnvPath });

// 2. Root .env.local (Shared Secrets: R2, Treasury)
const rootEnvPath = path.resolve(__dirname, '../../../.env.local');
dotenv.config({ path: rootEnvPath });

console.log("✅ Environment Loaded.");
console.log("   -> Service Env:", fs.existsSync(localEnvPath) ? "Found" : "Missing");
...
const SEED_DATA = {
    ...
    [CHANNELS.LOBBY]: {
        title: "🪐 THE LOBBY",
        desc: "**General Comms**\nOpen frequency for all initiates and travelers.\n\n- Be respectful.\n- No spam.\n- Check pins for official links.",
        image: null
    },
    [CHANNELS.ANNOUNCEMENTS]: {
        title: "📢 PROTOCOL SIGNALS",
        desc: "**Official Transmissions Only**\nAll critical updates regarding The Curve, The Vault, and System Upgrades will be broadcast here.",
        image: null
    },
    [CHANNELS.COUNCIL]: {
        title: "🏛️ THE COUNCIL",
        desc: "**Governance Chamber**\nAccessible only to **Diamond** Tier holders.\n\nHere we discuss the future trajectory of the protocol.",
        image: null
    },
    [CHANNELS.INNER_SANCTUM]: {
        title: "👁️ INNER SANCTUM",
        desc: "**System Logs & Admin Command**\nRestricted access. Monitoring system health, Treasury status, and Deployment protocols.",
        image: null
    }
};
...
let walletMap: Record<string, string> = {}; // Wallet -> DiscordID
...
app.get('/api/epoch', (req, res) => {
    ...
    res.json({
        ...
        target: nextTarget,
        nextReward: EPOCHS[currentEpoch as keyof typeof EPOCHS]?.reward || "Unknown"
    });
});
...
app.post('/api/notify', async (req, res) => {
    try {
        const { channelIdx, embed, content } = req.body;

        // Resolve Channel
        // channelIdx can be a key in CHANNELS (e.g., 'BOUNTIES') or a raw ID
        let targetId = CHANNELS[channelIdx as keyof typeof CHANNELS] || channelIdx;

        const guild = client.guilds.cache.get(GUILD_ID);
        if (!guild) return res.status(500).json({ error: "Guild not initialized" });

        const channel = guild.channels.cache.get(targetId) as TextChannel;
...
app.post('/api/verify', async (req, res) => {
    try {
        ...
        let tier = "None";
        if (gritBalance >= 1000 || moxyBalance >= 7000) tier = "Diamond";
        else if (gritBalance >= 500 || moxyBalance >= 3500) tier = "Gold";
        else if (gritBalance >= 250 || moxyBalance >= 1750) tier = "Silver";
        else if (gritBalance >= 100 || moxyBalance >= 700) tier = "Bronze";
        ...
        if (guild) {
            try {
                ...
                if (nucleusRole) await member.roles.add(nucleusRole);
...
client.on(Events.MessageCreate, async (message) => {
    ...
    // --- CHAT FORWARDING (Matrix Feed) ---
    // Forward specific channels to the Dashboard API
    const PUBLIC_CHANNELS = [CHANNELS.LOBBY, CHANNELS.ANNOUNCEMENTS, "123456789"]; // Add IDs

    // Check if channel is monitored (simple include check)
    // Using simple string matching for now since CHANNELS values are IDs
    const isMonitored = Object.values(CHANNELS).includes(message.channelId) || PUBLIC_CHANNELS.includes(message.channelId);
        ...
        try {
            await fetch("http://localhost:3000/api/chat", {
                ...
                body: JSON.stringify({
                    ...
                    channel: (message.channel as TextChannel).name || "unknown",
                    timestamp: Date.now()
                })
            });
        } catch (e) {
            // fail silently, dashboard might be down
        }
    }
...
// --- ENTITLEMENT HANDLER (Monetization) ---
client.on(Events.EntitlementCreate, async (entitlement) => {
    console.log(`💰 New Entitlement: ${entitlement.id}, SKU: ${entitlement.skuId}, User: ${entitlement.userId}`);

    // 1. Find SKU Config (by ID match or fallback for testing)
    // In production, match entitlement.skuId === config.id
    // For local test with placeholder IDs, we might fallback to a default if not found
    const skuKey = Object.keys(skuConfig.skus).find(key =>
        skuConfig.skus[key as keyof typeof skuConfig.skus].id === entitlement.skuId
    );

    // Test Mode Override: If no match, treat as SMALL_PACK for testing
    const config = skuKey ? skuConfig.skus[skuKey as keyof typeof skuConfig.skus] : skuConfig.skus.SKU_GRIT_SMALL;

    console.log(`   -> Product: ${config.name} (${config.type})`);

    // 2. Resolve User & Wallet
    // We need the Discord User -> Wallet mapping
    const wallet = Object.keys(walletMap).find(key => walletMap[key] === entitlement.userId);

    if (!wallet) {
        console.warn(`   ⚠️ User ${entitlement.userId} has no linked wallet. Cannot deliver tokens.`);
        try {
            const user = await client.users.fetch(entitlement.userId);
            await user.send("🚨 **Start Your Engines**: You purchased an item but your wallet is not linked!\nPlease verify with `!verify` to receive your rewards.");
        } catch (e) { }
        return;
    }
    ...
    try {
        // 3. Deliver Reward
        if (config.rewardType === "TOKEN") {
            const tokenConfig = config as { amount: number; token: string };
            console.log(`   -> Minting ${tokenConfig.amount} ${tokenConfig.token} to ${wallet}...`);

            // LOAD TREASURY / MINT AUTHORITY
            // For GRIT, the Treasury might have Authority, or we transfer from a supply?
            // Assuming Treasury holds supply or has mint authority. 
            // Let's assume Transfer from Treasury for now (safer).

            if (!treasuryKey) throw new Error("Treasury Key not loaded");

            const MINT_ADDRESS = tokenConfig.token === "GRIT"
                ? new PublicKey("CS8ZQMdJ5t5hNuM51LXJBU4zBysZWAkFj9oJ6MwtnHsS")
                : new PublicKey("2FFhBNoCqsgXejrqQXk3gJXWyG9nuiE7qj4Sv2wrcnwq"); // MOXY

            // Create ATAs
            const sourceATA = await getAssociatedTokenAddress(MINT_ADDRESS, treasuryKey.publicKey);
            const destATA = await getAssociatedTokenAddress(MINT_ADDRESS, new PublicKey(wallet));

            // Ensure dest ATA exists (handled by transfer? No, need create)
            ...
            await transfer(
                ...
                BigInt(tokenConfig.amount * 1_000_000_000) // 9 decimals
            );
            ...
            // Notify User
            const user = await client.users.fetch(entitlement.userId);
            await user.send(`🎁 **Delivery Confirmed**: ${tokenConfig.amount} ${tokenConfig.token} has been transferred to your wallet.`);

        } else if (config.rewardType === "ROLE") {
            const roleConfig = config as { roleName: string };
            // Assign Role
            const guild = client.guilds.cache.get(entitlement.guildId || GUILD_ID);
            if (guild) {
                const member = await guild.members.fetch(entitlement.userId);
                const role = guild.roles.cache.find(r => r.name === roleConfig.roleName);
                if (role) await member.roles.add(role);
                console.log(`   ✅ Role ${roleConfig.roleName} Assigned.`);
            }
        }

        // 4. Consume Entitlement (If Consumable)
        if (config.type === "CONSUMABLE") {
            console.log("   -> Consuming Entitlement...");
            // Consume via Discord API (Requires simple fetch since discord.js support might be partial)
            // Or usually discord.js entitlements manager handles this?
            // entitlement.consume() is available in newer versions.
            // If not, we skip for this demo or use raw API.
            // Checking types... discord.js v14.16 should have it. Await fallback.

            // @ts-ignore
            if (typeof entitlement.consume === 'function') {
                // @ts-ignore
                await entitlement.consume();
                console.log("   ✅ Consumed.");
            } else {
                console.log("   ⚠️ Consumable logic skipped (Method not found on object).");
            }
        }
        ...
        try {
            const user = await client.users.fetch(entitlement.userId);
            await user.send(`⚠️ **Delivery Error**: We couldn't deliver your ${config.name}. Please contact Admin. Ref: ${entitlement.id}`);
        } catch (err) { }
    }
});
        ...
        for (const wallet of wallets) {
            try {
                ...
                const [userStakePDA] = PublicKey.findProgramAddressSync(
                    ...
                    new PublicKey("G9Xq99jdwuvQD1nGGhW1C3TYuc6iRz78faoscQqmX2D7") // Staking Program
                );
...
async function startSentinel() {
    ...
    if (user) await user.send("🛡️ **System Online**: Sentinel is now monitoring the Saturn Protocol.");
...
async function startNucleusRadio() {
    ...
    voiceConnection.on(VoiceConnectionStatus.Ready, async () => {
        ...
        } catch (e) { console.error("Failed to undeafen:", e); }
    });
    ...
    const playNext = async () => {
        try {
            ...
            if (!trackKey) {
                console.warn("⚠️ Radio: Track key undefined. Skipping.");
                trackIndex = 0; // Reset
                setTimeout(playNext, 1000);
                return;
            }

            // GENERATE SIGNED URL (Bypasses Public Access / Encoding Issues)
            let trackUrl = "";
            if (r2) {
                try {
                    const command = new GetObjectCommand({ Bucket: "wzrdclvb", Key: trackKey });
                    trackUrl = await getSignedUrl(r2, command, { expiresIn: 3600 });
                } catch (e) {
                    console.error("Failed to sign URL:", e);
                }
            }

            // Fallback to Public URL if signing fails (or r2 missing)
            if (!trackUrl) {
                trackUrl = `${R2_PUBLIC_URL}/${encodeURIComponent(trackKey).replace(/%2F/g, '/')}`;
            }
            ...
            // Use Arbitrary to avoid probing lag/failures
    ...
    player.on(AudioPlayerStatus.Playing, () => {
        console.log('▶️ Player Status: Playing');
    });
    ...
    player.on(AudioPlayerStatus.AutoPaused, () => {
        console.log('kB Player Status: AutoPaused');
    });

    player.on(AudioPlayerStatus.Idle, () => {
        console.log('⏹️ Player Status: Idle (Track Finished)');
        // Add a small delay to prevent CPU spinning if it insta-skips
        setTimeout(playNext, 1000);
    });
